...
Jussi Jousimo, jussi.jousimo@helsinki.fi
...
public class FractalPlot extends java.applet.Applet {
	/* Ohjausikkunaolio */
	...
	/* K‰ynnist‰‰ sovelluksen. Metodi kutsuu sovelman k‰ynnistysmetodia,
...
class Complex {
	...
	/* Korottaa kompleksiluvun toiseen */
	public Complex pow2() {
...
/* Luokka Fractal on abstrakti yliluokka kaikille eri tyyppisille
   ...
   aktiivisena vain yksi kerralla. */
abstract class Fractal implements Runnable { 
	...
	public void plot(Graphics screen, Graphics image) /*throws Exception*/ {
		...
		/* Extratarkistus */
		if (stopPlot==true && doesPlot()==false) stopPlot=false;
	...
	public static boolean doesPlot() {
		if (plotThread==null) return false;
...
class ControlFrame extends Frame implements ActionListener, ItemListener, 
	...
	/* Palauttaa k‰ytt‰j‰n syˆtt‰m‰t zoomauskoordinaatit. Jos syˆte on
	   virheellinen, aiheuttaa poikkeuksen. */
	...
	/* Palauttaa k‰ytt‰j‰n syˆtt‰m‰n maksimi-iteraatioiden m‰‰r‰n. Jos
	   syˆte on virheellinen, aiheuttaa poikkeuksen. */
	...
	public void itemStateChanged(ItemEvent e) {
		...
		if (o==checkMandelbrot) setMandelbrot();
		else if (o==checkJulia) setJulia();
	}
...
class PlotFrame extends Frame implements WindowListener, MouseListener, 
	...
	public void paint(Graphics g) {
 		if (paintArea==null || image==null) return;
	...
	public void mouseDragged(MouseEvent e) {
		if ((e.getModifiers()&MouseEvent.BUTTON1_MASK)!=0) {
			...
			if (mousePosition!=null) {
				...
				if (currentPosition!=null) g.drawRect(x0, y0, x1-x0, y1-y0);
				...
				if (x0-paintArea.x<0) x0=paintArea.x;
				if (x1-paintArea.x>=paintArea.width) 
					x1=paintArea.width+paintArea.x-1;
				if (y0-paintArea.y<0) y0=paintArea.y;
